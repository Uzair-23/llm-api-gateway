...
## Current Status (paste this into Copilot Chat at the start of each session)
...
### Phase 1 — Tenant Auth
- [ ] `/auth/signup` implemented (bcrypt password hash, Mongoose Tenant schema, zod validation)
- [ ] `/auth/login` implemented (returns JWT)
- [ ] Verified: signup → login → JWT → hit protected dummy route
- [ ] Committed: `feat(phase-1): tenant signup/login with JWT`
- **Date completed:** ___________
- **Notes:** ___________

### Phase 2 — API Key Issuance + Auth Middleware
- [ ] API key generation on signup (`sk-live-{32 hex}` format, bcrypt hash stored)
- [ ] `auth.middleware.ts` — API key auth with Redis cache-aside (5 min TTL) → MongoDB fallback
- [ ] Verified: valid key passes, invalid key → 401, repeated request within TTL doesn't hit MongoDB
...
- **Date completed:** ___________
- **Notes / actual numbers:** ___________
